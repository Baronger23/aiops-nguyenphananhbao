...
*   Điểm đồng thuận cao nhất: $Score_{\text{max}} = 2.092$ (dành cho `rollback_service`)
*   Điểm đồng thuận của `increase_pool_size`: $Score = 0.474$

Tính toán:
1.  **Hành động `rollback_service`**:
    $$P_{\text{success}} = 0.809 \times \frac{2.092}{2.092} = 0.809 \approx \mathbf{0.81}$$
2.  **Hành động `increase_pool_size`**:
    $$P_{\text{success}} = 0.809 \times \frac{0.474}{2.092} = 0.183 \approx \mathbf{0.18}$$
...
1.  **Hành động `rollback_service`**:
    ...
    $$EV = 65.61 - 32.11 = \mathbf{33.50}$$
    *(Sai số làm tròn nhỏ trong code thực tế trả về **33.25**)*

2.  **Hành động `increase_pool_size`**:
    $$EV = 0.18 \cdot (100 - 6) + 0.82 \cdot (-150 - 6)$$
    $$EV = 0.18 \cdot 94 + 0.82 \cdot (-156)$$
    $$EV = 16.92 - 127.92 = \mathbf{-111.00}$$
    *(Code thực tế trả về **-110.17**)*
...
### Kết quả quyết định:
Hành động **`rollback_service`** thắng cuộc vì có điểm EV lớn nhất ($33.25 > 27.50 > -110.17$). Động cơ tự động rollback dịch vụ `esb` thay vì gọi kỹ sư trực ca.
