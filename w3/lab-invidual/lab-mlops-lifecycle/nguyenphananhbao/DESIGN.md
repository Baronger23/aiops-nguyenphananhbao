...
`--check-mode combined` chạy song song 2 cơ chế: (1) Evidently `DataDriftPreset` trên feature distribution, và (2) đánh giá precision/recall của model hiện tại trên `drifted.csv` (qua tham số `--labeled-current` chứa nhãn thực tế bị lật 25% do concept drift). Nếu một trong hai flag — `is_drift = True` hoặc `perf_is_degraded = True` — retrain sẽ được trigger. Ngưỡng performance mặc định là precision ≥ 0.70.

**Số liệu thực tế đo được từ run (Stress 1):**
* Đối với model v1 (`anomaly-detector` v8) trên tập dữ liệu `drifted.csv`:
  * **Drift score**: `1.0000` (Wasserstein distance phát hiện trôi lệch 3/3 features).
  * **Perf precision**: `0.2907` (so với ngưỡng `0.70`), giảm sút từ mức 91% ban đầu do Concept Drift (25% nhãn bị lật).
  * **Perf recall**: `1.0000`.
  * **Perf degraded**: `True`.
...
**Số liệu thực tế đo được từ run (Stress 2):**
* Kích thước tập gộp (sliding window): **5328 dòng**.
* Đánh giá v2 (`anomaly-detector` v9) trên tập `holdout.csv` in ra dòng chính xác:
  `Holdout validation — v2 precision: 0.0000  recall: 0.0000`
* Hiệu năng của v2 hoàn toàn tương đương v1 trên tập holdout này (cả hai đều có precision/recall là 0.0000 do không có điểm dị thường nào được phân loại trên tập holdout siêu sạch này), đảm bảo hiệu năng không bị giảm sút.
...
Tại sao 0.65? Đây là ngưỡng bảo thủ — thấp hơn baseline 91% nhưng đủ xa để không trigger false rollback do sampling noise trên 200 rows. Ngưỡng 0.65 nằm ở điểm "model rõ ràng đang sai lệch nghiêm trọng".

Rollback flow: `client.set_registered_model_alias(MODEL_NAME, "archived", v2_version)` -> `client.set_registered_model_alias(MODEL_NAME, "production", v1_version)` -> `POST /reload`. Toàn bộ < 5 giây.

**Số liệu thực tế và audit log ghi nhận từ run (Stress 3):**
* Khi theo dõi model v2 (`version 9`) trên `post_deploy_eval.csv`, ở chu kỳ giám sát thứ nhất, độ chính xác đạt:
  `[post_deploy_monitor] Cycle 01/24 — precision: 0.4000  recall: 1.0000`
* Vì `0.4000 < 0.65`, cơ chế tự động rollback ngay lập tức kích hoạt để khôi phục v1 (`version 8`) làm `@production` và giáng cấp v2 (`version 9`) xuống `@archived`.
* File nhật ký `outputs/audit_log.jsonl` ghi nhận sự kiện thực tế như sau:
  ```json
  {"timestamp": "2026-06-19T04:37:02.247003", "event": "auto_rollback_v2_to_v1", "demoted_version": "9", "restored_version": "8", "trigger_precision": 0.4, "threshold": 0.65, "cycle": 1}
  ```
