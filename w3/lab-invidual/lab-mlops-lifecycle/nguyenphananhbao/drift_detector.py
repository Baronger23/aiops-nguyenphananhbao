"""
drift_detector.py — Chạy Evidently DataDriftPreset và kiểm tra hiệu năng (Concept Drift).
"""
...
DEFAULT_PERF_THRESHOLD = 0.70   # Precision tối thiểu trên tập holdout có nhãn
REPORT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "outputs", "drift_reports")


@dataclass
class DriftResult:
    score: float            # tỷ lệ phần trăm các features bị drift (0.0–1.0)
    is_drift: bool
    threshold: float
    drifted_features: list[str]
    report_path: str
    timestamp: str
    # Các trường kiểm tra hiệu năng
...
def check_performance_drift(
    ...
    """Đánh giá model precision/recall trên tập dữ liệu có nhãn để phát hiện concept drift."""
...
def log_to_mlflow(result: DriftResult, experiment_name: str = "anomaly-detection-drift") -> None:
    """Log drift score vào MLflow để theo dõi xu hướng."""
...
def main():
    parser = argparse.ArgumentParser(description="Detect data drift and concept drift")
