...
case "$FAULT" in
  latency)
    ...
    DELAY_MS="${DELAY//ms/}"
    docker run --rm --privileged --pid=host alpine sh -c "apk add --no-cache iproute2 >/dev/null 2>&1 && nsenter -t $PID -n -- tc qdisc add dev eth0 root netem delay ${DELAY_MS}ms 2>/dev/null || nsenter -t $PID -n -- tc qdisc change dev eth0 root netem delay ${DELAY_MS}ms"
  ...
  clear-latency)
    ...
    PID=$(docker inspect --format '{{.State.Pid}}' "$CONTAINER")
    docker run --rm --privileged --pid=host alpine sh -c "apk add --no-cache iproute2 >/dev/null 2>&1 && nsenter -t $PID -n -- tc qdisc del dev eth0 root 2>/dev/null || true"
